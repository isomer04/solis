...
// Schema for the CryptoCompare price endpoint: { [SYMBOL]: number }
const PriceResponseSchema = z.record(z.string(), z.number());
...
export default function Converter() {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((r) => r.json())
      .then((json: unknown) => {
        const parseResult = PriceResponseSchema.safeParse(json);
        if (!parseResult.success) {
          setError("Pair not found. Check your symbols.");
          setResult(null);
          return;
        }
        const rate = parseResult.data[to];
