...
import { PostIdParamSchema } from "../schemas";
...
function parseComments(raw: unknown[]): Comment[] {
  return raw.flatMap((item, index) => {
    ...
    if (typeof item === "string") {
      try {
        const parsed = JSON.parse(item) as { id?: string; text?: string } | null;
...
export default function Post() {
  ...
  // Validate the URL param — reject non-integer or negative ids immediately
  const idResult = PostIdParamSchema.safeParse(rawId);
  const currentPost = idResult.success
    ? posts.find((p) => p.id === idResult.data)
    : undefined;
  ...
  const comments = useMemo(
    () => parseComments(currentPost?.comments ?? []),
  ...
  const handleUpvote = () => {
    void startUpvoteTransition(async () => {
  ...
  const handleComment = (e: React.SyntheticEvent<HTMLFormElement, SubmitEvent>) => {
    ...
    void startCommentTransition(async () => {
